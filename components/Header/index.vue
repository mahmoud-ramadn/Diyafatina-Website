<template>
  <header class="mt-6 md:mt-10 relative">
        ...
        <ul class="flex items-center gap-6 md:gap-10">
          <li v-for="(item,index) in navLinks" :key="index">
            <nuxt-link 
              ...
              to="#"  
              class="hover:text-primary/80 transition-all duration-150 text-sm md:text-base" :class="[
    ...
    <div v-if="isMenuOpen" class="md:hidden absolute top-full left-0 right-0 bg-white py-4 px-4 shadow-lg z-50">
      <ul class="flex flex-col items-center gap-4">
        <li v-for="(item,index) in navLinks" :key="`mobile-${index}`">
          <nuxt-link 
            to="#"  
            class="hover:text-primary/80 transition-all duration-150 text-lg" :class="[
              index < 1 ? 'text-primary' : 'text-gray-500'
            ]"
      ...
      <div class="py-5 w-full flex items-center justify-center gap-3">
        <a href="#" target="_blank" class="md:w-auto">
          <img src="~/assets/images/googleplay.webp" alt="googlePlay" class="h-8 md:h-10 shrink-0 w-[100px] md:w-[122px]">
        </a>
        <a href="#" target="_blank">
          <svg-icon name="app_stroe.svg" class="h-8 md:h-10 w-[100px] md:w-[122px]" />
        </a>
      </div>
    </div>
  </header>
